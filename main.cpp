...
int main()
{
	...
	Scenario scn = Scenario(id, -1, 3, 3, Robots, Carts, pose_dest, cart_selected);
		...
		if (scn_sel.value == 0) {
			...
			for (int i = 0; i < scn_sel.robots.size(); i++) {
				...
				if (pose_valid(scn_sel, new_pose) &&
					...
					//es m�s f�cil clonar.
					id++;
					Scenario newscn = Scenario(id, scn_sel.id, scn_sel.height, scn_sel.width, scn_sel.robots, scn_sel.carts, pose_dest, cart_selected);
					//modificar
